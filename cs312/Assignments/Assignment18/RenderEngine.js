...
RenderEngine.prototype.drawLine = function(P1, P2, fragmentShader)
{
   ...
   if (maxDist > 1)
   {
      for (var i = 1; i < maxDist; i++)
      {
         ...
         this.renderBuffer.setPixel(point[0][0], point[0][1], point[0][2], point[1]);
      }
   }
   
   // Draw the 2 end-points
   this.renderBuffer.setPixel(P1[0][0], P1[0][1], P1[0][2], fragmentShader.main(P1[1]));
   this.renderBuffer.setPixel(P2[0][0], P2[0][1], P2[0][2], fragmentShader.main(P2[1]));
}
...
{
   var remainingPoly = new Array();
   var wasIn = false;

   if (plane.pointInPlane(poly[0][0]) > 0)
   {
...
RenderEngine.prototype.clipPolygon = function(verts)
{
   var clipVerts = verts;
   for (var clipIdx = 0; clipIdx < clipPlanes.length; clipIdx++)
   {
      if (verts.length == 0) return;
      clipVerts = this.clipPolygonAgainstPlane(clipVerts, clipPlanes[clipIdx]);
   }
   return clipVerts;
}
...
RenderEngine.prototype.checkDrawTriangle = function(vertArray, winding,
                                                    fragmentShader)
{
   vertArray = this.clipPolygon(vertArray);
   for (var i = 0; i < vertArray.length; i++)
   {
      vertArray[i][0] = this.ndc2screen(vertArray[i][0]);
   }

   for (var i = 2; i < vertArray.length; i++)
   {
      var triangle = [vertArray[0], vertArray[i - 1], vertArray[i]];
   ...
   if (maxDistL > 1 && maxDist1 > 1 && maxDist2 > 1)
   {
      ...
      for (var i = 1; i <= maxDistL; i++)
      {
         ...
         while (lineS.getY() < lineL.getY())
         {
            ...
            this.renderBuffer.setPixel(pointS[0][0], pointS[0][1], pointS[0][2], pointS[1]);
         }

         // takes the two points found (with equal y values) on the screen
         // and displays them using drawLine()
         //console.log("drawTriangle " + pointL[0]);
         this.drawLine(pointL, pointS, fragmentShader);
      }
   }

   // display the three vertices of the triangle using setPixel()
   this.renderBuffer.setPixel(vertArray[0][0][0], vertArray[0][0][1], vertArray[0][0][2],
                              fragmentShader.main(vertArray[0][1]));
   this.renderBuffer.setPixel(vertArray[1][0][0], vertArray[1][0][1], vertArray[1][0][2],
                              fragmentShader.main(vertArray[1][1]));
   this.renderBuffer.setPixel(vertArray[2][0][0], vertArray[2][0][1], vertArray[2][0][2],
...
RenderEngine.prototype.setBackground = function(color)
{
   for (var i = 0; i < this.renderBuffer.width; i++)
   {
      ...
      {
         this.renderBuffer.setPixel(i, j, 999999, color);
      }
   }
}
