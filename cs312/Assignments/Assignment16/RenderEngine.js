                                             ...
                                             fragmentShader)
{
   for (var i = 0; i < verts.length; i++)
   {
      for (var i = 0; i < 6; i++)
      {
         if (clipPlanes[i].pointInPlane(verts[i]) < 0) return;
      }
...
RenderEngine.prototype.clipLineToPlane = function(point1, point2, plane)
{
   if ((plane.pointInPlane(point1[0]) >= 0) && (plane.pointInPlane(point2[0]) >= 0))
   {
      return true;
   }
   if ((plane.pointInPlane(point1[0]) < 0) && (plane.pointInPlane(point2[0]) < 0))
   ...
   var intersection = findIntersectVertex(plane, point1, point2);
   if (intersection == false) return false;
   if (plane.pointInPlane(point1[0]) < 0) point1 = intersection;
   else if (plane.pointInPlane(point2[0]) < 0) point2 = intersection;
   return true;
}

RenderEngine.prototype.clipLine = function(point1, point2, fragmentShader)
{
   for (var i = 0; i < 6; i++)
   {
      if (this.clipLineToPlane(point1, point2, clipPlanes[i]) == false) return;
   }
   point1[0] = this.ndc2screen(point1[0]);
   point2[0] = this.ndc2screen(point2[0]);
   this.drawLine(point1, point2, fragmentShader);
}
                                               ...
                                               translatedVerts, varyingVars)
{
   ...
   for (var i = 0; i < verts.length; i++)
   {
      ...
      transVert = vertexShader.main(verts[i]);
      var iW = 1 / transVert[3];
      transVert[0] *= iW;
      transVert[1] *= iW;
      transVert[2] *= iW;
      transVert[3] *= iW;
      translatedVerts.push(transVert);
