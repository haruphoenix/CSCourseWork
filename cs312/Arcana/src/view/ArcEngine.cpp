...
ArcMatrix gModelMatrix;      
ArcMatrix gViewMatrix;       
ArcMatrix gProjectionMatrix; 
...
void ArcEngine::resize(int width, int height)
{
  ...
  gProjectionMatrix = CreateProjectionMatrix(60,
					     (float)mWidth / mHeight,
					     1.0f, 1000.0f);
  mShaderProgram.setMatrix(gProjectionMatrix);
}
/*
void ArcEngine::moveCamera(int pX, int pY)
{
  mCamera.changeDirection(pX, pY);
}
*/
/*
 * Initialize OpenGL
...
bool ArcEngine::initialize()
{
  ...
  gModelMatrix      = IDENTITY_MATRIX;
  gProjectionMatrix = IDENTITY_MATRIX;
  gViewMatrix       = IDENTITY_MATRIX;
  TranslateMatrix(&gViewMatrix, 0, 0, -2.0f);
...
void ArcEngine::render()
{
  ...
  mShaderProgram.run(mCanvas, gModelMatrix, mCamera.getMatrix());//gViewMatrix);//
}
