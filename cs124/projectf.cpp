...
*    in green, and will solve a board
*    
*    Estimated:  20.0 hrs   
*    Actual:     16.5 hrs
*      Solving the board was surprisingly very easy.
************************************************************************/
...
void solveBoard(int sudoku[][COL_SIZE]);
...
/**********************************************************************
* Solves the board for you
***********************************************************************/
void solveBoard(int sudoku[][COL_SIZE])
{
   // Set board to not solved
   bool isSolved = false;
   ...
   while (!isSolved)
   {
      // Say it's solved until something determines it's not solved
      isSolved = true;
      ...
      for (int i = 0; i < ROW_SIZE; i++)
      {
         for (int j = 0; j < COL_SIZE; j++)
         {
            ...
            if (sudoku[i][j] == 0)
            {
               ...
               // See if there's only 1 possible value
               if (values[1] == 0)
               {
                  // If there is, set that square to only value
                  sudoku[i][j] = -values[0];
               }

               // Otherwise, that square still isn't solved
               else
               {
                  isSolved = false;
               }
      ...
      // Check to see if it's solvable
      count++;
      if (count > (ROW_SIZE * COL_SIZE))
      {
         cout << "ERROR: Unable to solve board.\n";
         return;
      }
   }
}
