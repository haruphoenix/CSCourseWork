...
*    Haru McClellan
* Summary:
*    This program moves a bunch of rectangles all over the screen.
*    It prompts the user for the number of frames, the number of
*    rectangles, and the speed of the rectangles
***********************************************************************/
...
void moveObjects(vector<Figure> &figures, int numFrame, int &delay);
...
int main()
{
   ...
   while (drawCnt > 0)
   {
      ...
      drawCnt = getInt("Number of times to draw object (0 to exit): ", 10000, 
                      "Invalid Entry - must be a number 0 - 10000!\n");
      ...
      figureCnt = getInt("Number of figures (0 - 50): ", 50,
                         "Invalid Entry - must be a number 1 - 50!\n");
      ...
      {
         Figure rectangle(row, col);     
         figureVec.push_back(rectangle);
         row += 10;
         if (row > 40)
         {
            row = 0;
      ...
      moveObjects(figureVec, drawCnt, delay);
...
void moveObjects(vector<Figure> &figures, int numFrame, int &delay)
{
   ...
   if (numFrame != 0)
   {
      ...
      refresh();
      pause(delay);
      moveObjects(figures, numFrame, delay);
   }
   return;
}
