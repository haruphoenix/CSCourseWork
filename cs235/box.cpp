...
void moveObjects(vector < Figure > &figures);
...
int main()
{
   ...
   while (drawCnt > 0)
   {
      ...
      {
         moveObjects(figureVec);  // Move the object to next position
...
void moveObjects(vector < Figure > &figures)
{
   ...
   change = figures.front().getRowChange();
   colChange = figures.front().getColChange();
   
   // calculate the new row and col positions;
   row = figures.front().box.getRow() + change;
   col = figures.front().box.getCol() + colChange;
   ...
   figures.front().setRowChange(change);
   figures.front().setColChange(colChange);
   figures.front().box.move(row, col);
}
