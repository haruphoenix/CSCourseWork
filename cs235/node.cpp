...
/******************************************************************
* Application specific: overloading of operator to display list
* of integers less than 100, spaced evenly apart, twenty numbers
* per line.
******************************************************************/
template <class T>
ostream& operator << (ostream& out, LList<T>& theList)
{
   for (int i = 0; i < theList.size(); i++)
   {
      out << setw(3) << theList[i];
      if ((((i + 1) % 20) == 0) && (i != theList.size() - 1))
         out << endl;
   }
   return out;
}
...
/******************************************************************
* Reads in a file from command line, and inserts numbers from it
* into a linked list in ascending order. Prints the list.
******************************************************************/
int main(int argc, char** argv)
{
   ...
   try
   {
      if (argc < 2)                      // Make sure at least
         throw argc;                     // one file is given
      for (int i = 1; i < argc; i++)
      {
         read.open(argv[i]);
         if (read.fail())                // Check for valid file
            throw argv[i];
         while (read >> input)           // Read numbers from file
         {
            inserted = false;
            for (int i = 0; i < myList.size(); i++)
            {
               if (input <= myList[i])// Insert in ascending order
               {
                  myList.setCurrent(i - 1);
                  if (i == 0)
                     myList.headInsert(input);
                  else
                     myList.insert(input);
                  inserted = true;
                  break;
            ...
            if (!inserted)
            myList.backInsert(input);
         }
         cout << myList << endl;         // Print list
         myList.empty();                 // Empty list
      }
   }
   catch (int e)
   {
